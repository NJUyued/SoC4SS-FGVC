...
def make_dataset(dataset_root, split, task='All', pl_list=None):
    if 'u' in split:
        split_file_path = os.path.join(dataset_root, task, split+'_oracle.txt')
    else:
    ...
    for idx, x in enumerate(img):
        if task == 'semi_fungi':
            ...
            img[idx][0] = os.path.join(dataset_root, x[0].split('/')[0], x[0])
        img[idx][1] = int(x[1])
...
class iNatDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.return_name:
            ...
            return img, self.strong_transform(img_original), target, index
